...
int read_image_data_csv( char *filename, std::vector<char *> &filenames, std::vector<std::vector<float>> &data, int echo_file ) {
  ...
  for(;;) {
    ...
    // Attempt to read the filename
    int eol = getstring( fp, img_file );
    
    // Check if the line is empty (only contains EOL or EOF) or if the filename is empty
    if(eol || strlen(img_file) == 0) {
      if (feof(fp)) {
        // If we've reached the end of the file, break out of the loop
        break;
      }
      // Otherwise, this is an empty line, so skip processing it and move to the next line
      continue;
    }

    // If getstring did not return an empty line, proceed with reading the rest of the data
    for(;;) {
      // Get next feature
